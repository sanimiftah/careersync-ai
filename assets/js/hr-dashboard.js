...
class HRDashboard {
  constructor() {
    console.log('HRDashboard constructor called');
    this.currentUser = JSON.parse(localStorage.getItem('careersync_user')) || this.createDemoUser('hr');
  ...
  loadUserData() {
    ...
    const userName = this.currentUser.firstName || this.currentUser.name || 'HR Manager';
    
    if (displayName) {
      displayName.textContent = userName;
    }
    if (hrName) {
      hrName.textContent = userName;
    }
  }
  ...
  loadDashboardData() {
    ...
    this.loadRecentJobs();
    this.loadHiringPipeline();
    this.loadAnalyticsInsights();
    this.updateStats();
  }

  loadTopCandidates() {
    console.log('Loading top candidates...');
  ...
  loadRecentJobs() {
    ...
    console.log('Recent jobs container found:', container);
    if (!container) return;
    ...
    container.innerHTML = jobs.map(job => `
      <div class="p-4 border border-gray-200 rounded-lg hover:border-blue-500 transition-colors">
        <div class="flex items-start justify-between mb-2">
          ...
          <span class="bg-${job.priority === 'High' ? 'red' : job.priority === 'Medium' ? 'yellow' : 'green'}-100 text-${job.priority === 'High' ? 'red' : job.priority === 'Medium' ? 'yellow' : 'green'}-800 text-xs px-2 py-1 rounded">${job.priority}</span>
        </div>
        <div class="flex items-center justify-between text-sm">
          <span class="text-gray-600">${job.applications} applications • Posted ${job.posted}</span>
          <button onclick="viewJobDetails('${job.title}')" class="text-blue-600 hover:text-blue-700">View Details</button>
        </div>
      </div>
    `).join('');
  }
  ...
  loadHiringPipeline() {
    const container = document.getElementById('hiringPipeline');
    if (!container) return;
    ...
    container.innerHTML = pipeline.map(stage => `
      <div class="flex items-center justify-between p-3 border border-gray-200 rounded-lg mb-2">
        <div class="flex items-center">
          <div class="w-3 h-3 bg-${stage.color}-500 rounded-full mr-3"></div>
    ...
    container.innerHTML = insights.map(insight => `
      <div class="p-3 border border-gray-200 rounded-lg">
        <div class="flex items-center justify-between mb-1">
          ...
          <span class="text-${insight.trend === 'up' ? 'green' : 'red'}-600 text-xs font-medium">${insight.change}</span>
        </div>
        <div class="text-lg font-bold text-gray-900">${insight.value}</div>
      </div>
    `).join('');
  }
  ...
  updateStats() {
    ...
    const activeCandidates = document.getElementById('activeCandidates');
    ...
    if (activeCandidates) activeCandidates.textContent = '247';
    if (openPositions) openPositions.textContent = '12';
    if (interviewsScheduled) interviewsScheduled.textContent = '18';
...
document.addEventListener('DOMContentLoaded', () => {
  console.log('DOM loaded, initializing HR Dashboard...');
  new HRDashboard();
});
